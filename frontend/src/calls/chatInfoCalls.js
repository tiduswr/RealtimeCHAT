...
const fetchUserImage = async (username) => {
    const imageResponse = await Api.get(`/users/retrieve_profile_image/${username}`, { responseType: 'arraybuffer' });
    return URL.createObjectURL(new Blob([imageResponse?.data], { type: 'image/png' }));
}

const fetchUnreadedMessageCount = async (username) => {
...
const buildContact = async (username) => {
    const image = await fetchUserImage(username);
    const data = await fetchUserInfo(username);
    return {...data, image}
}

export { fetchUnreadedMessageCount, fetchUserImage, fetchUserInfo, buildContact }